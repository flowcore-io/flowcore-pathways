...
Deno.test({
  ...
  fn: async (t) => {
    ...
    await t.step("Pathway Writing - Valid Event", async () => {
      ...
      assertEquals(
        request.url,
        `http://localhost:${server.port}/event/test-tenant/test-data-core/test-flow-type/test-event-type`,
      )
    })
    ...
    await t.step("Pathway Writing - Invalid Event - Missing data object", async () => {
      ...
      try {
        ...
        if (error instanceof Error) {
          assertEquals(error.message, "Invalid data for pathway test-flow-type/test-event-type. Required")
        } else {
          throw error
        }
      }
